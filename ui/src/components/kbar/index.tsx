...
function buildPublicNavItems() {
  return navItems
    ...
    .map((item) => ({
      ...
      items:
        item.items?.filter(
          (child) => !child.access?.requireOrg && !child.access?.permission && !child.access?.role
        ) ?? []
    }));
}
...
export default function KBar({ children }: { children: React.ReactNode }) {
  if (!clerkAuthEnabled) {
    return <LocalKBar>{children}</LocalKBar>;
  }

  return <ClerkKBar>{children}</ClerkKBar>;
}

function ClerkKBar({ children }: { children: React.ReactNode }) {
...
function LocalKBar({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  const filteredItems = buildPublicNavItems();
