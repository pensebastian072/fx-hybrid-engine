...
OUT_DIR="${PROJECT_ROOT}/outputs/backtests/cloud_real_${RUN_ID}"
...
lean cloud backtest "${LEAN_PROJECT_ID}" \
  --name "fxle_real_${RUN_ID}" \
  --parameter backtest_config backtest_real.yaml \
  --push \
  --open \
  > "${OUT_DIR}/lean_cloud_real.log"
